...
structures::LinkedList<T> structures::LinkedList<T>::clone() {
    ...
    for (std::size_t i = 0; i < size_; i++) {
        list_clone.push_back(current->data());
...
structures::LinkedList< structures::LinkedList<T> * >
    ...
    list_halve.push_back(half1);
    list_halve.push_back(half2);
    Node* current = head;
    for (std::size_t i = 0; i < size_; i++) {
        std::size_t l = (i % 2 == 0) ? 0 : 1;
        list_halve[l]->push_back(current->data());
        current = current->next();
    }
    return list_halve;
}
...
T& structures::LinkedList<T>::at(std::size_t index) const {
    if (index >= size())
        throw std::out_of_range("Invalid index or empty list!");
    T a = index;
    Node* current = index == 0? head : before_index(a)->next();
    return current->data();
}
...
template<typename T>
T& structures::LinkedList<T>::operator[](std::size_t index) const {
    T a = index;
    return at(a);
}
